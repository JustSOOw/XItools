...
1.  **卡片摘要展示**：
    ...
    *   **截止日期**：如果设置了。
    *   **标签**：简短的分类标签。
    *   **子任务数量/进度**：例如 "3/5 个子任务完成"。
    *   **（可选）父任务/子任务层级指示**：例如通过缩进、图标或简要提示（如"子任务数"）表明其在任务层级中的位置。
...
3.  **点击卡片查看/编辑详情**：
    ...
    *   详情面板应包含以下可编辑内容：
        ...
        *   **子任务 (Sub-tasks)**：支持创建、查看、编辑、删除和标记完成层级化的子任务（即子任务可以有自己的子任务，形成树状结构）。在详情视图中清晰展示任务层级关系（例如使用缩进列表或树形控件）。
        *   **截止日期 (Due Date)**
        *   **负责人 (Assignee)**
        *   **优先级 (Priority)**
        *   **标签 (Tags)**
