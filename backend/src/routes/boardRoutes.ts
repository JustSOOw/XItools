...
export default async function boardRoutes(fastify: FastifyInstance) {
  // 获取工作区下的所有看板
  fastify.get('/workspaces/:workspaceId/boards', async (request, reply) => {
    try {
  ...
  // 获取项目下的所有看板
  fastify.get('/projects/:projectId/boards', async (request, reply) => {
    try {
  ...
  // 移动看板
  fastify.post('/boards/:id/move', async (request, reply) => {
    try {
      const { id } = request.params as { id: string };
      ...
      const board = await boardService.moveBoard(id, targetWorkspaceId, targetProjectId);
  ...
  // 重新排序看板（工作区下）
  fastify.post('/workspaces/:workspaceId/boards/reorder', async (request, reply) => {
    try {
      const { workspaceId } = request.params as { workspaceId: string };
  ...
  // 重新排序看板（项目下）
  fastify.post('/projects/:projectId/boards/reorder', async (request, reply) => {
    try {
      const { projectId } = request.params as { projectId: string };
      const { itemIds } = reorderSchema.parse(request.body);
  ...
  // 复制看板
  fastify.post('/boards/:id/duplicate', async (request, reply) => {
    try {
      const { id } = request.params as { id: string };
      ...
      const board = await boardService.duplicateBoard(id, newName, targetWorkspaceId, targetProjectId);
  ...
  // 获取看板统计信息
  fastify.get('/boards/:id/stats', async (request, reply) => {
    try {
      const { id } = request.params as { id: string };
  ...
  fastify.get('/board/current', async (request, reply) => {
    try {
      // 返回默认工作区的第一个看板，保持向后兼容
      const { workspaceService } = await import('../services/workspaceService');
      const defaultWorkspace = await workspaceService.getDefaultWorkspace();
