...
export class AuthService {
   ...
   * 用户注册
   */
  async register(data: UserRegisterRequest, userAgent?: string, ipAddress?: string): Promise<AuthResponse> {
    // 验证输入数据
   ...
   * 用户登录
   */
  async login(data: UserLoginRequest, userAgent?: string, ipAddress?: string): Promise<AuthResponse> {
    // 验证输入数据
   ...
   * 创建用户会话记录
   */
  private async createSession(userId: string, token: string, userAgent?: string, ipAddress?: string): Promise<UserSession> {
