 ...
 * 检查速率限制
 */
function checkRateLimit(apiKeyId: string, window: 'minute' | 'hour' | 'day'): {
  allowed: boolean;
  remaining: number;
  resetTime: number;
} {
...
export async function mcpAuthMiddleware(
  ...
  try {
    ...
    if (endpoint.includes('POST') || endpoint.includes('PUT') || endpoint.includes('DELETE')) {
      ...
      if (!userApiKey.permissions.includes(writePermission) && !userApiKey.permissions.includes('admin')) {
      ...
      if (!userApiKey.permissions.includes(requiredPermission) && !userApiKey.permissions.includes('admin')) {
        await reply.status(403).send({
          success: false,
          error: '权限不足，需要读取权限',
          code: 'INSUFFICIENT_PERMISSIONS'
