...
export function useAuthError() {
   ...
   * 获取用户友好的错误消息
   */
  const getErrorMsg = useCallback((error: any): string => {
   ...
   * 处理注册错误
   */
  const handleRegisterError = useCallback((error: any): string => {
   ...
   * 处理登录错误
   */
  const handleLoginError = useCallback((error: any): string => {
    const errorInfo = handleError(error);
   ...
   * 处理Token错误
   */
  const handleTokenError = useCallback((error: any): string => {
    const errorInfo = handleError(error);
   ...
   * 处理权限错误
   */
  const handlePermissionError = useCallback((error: any): string => {
    const errorInfo = handleError(error);
   ...
   * 处理资料更新错误
   */
  const handleProfileError = useCallback((error: any): string => {
    const originalError = error.response?.data?.error || '';
