...
const QuickActions: React.FC<QuickActionsProps> = ({
  ...
  const priorityOptions = [
    { value: 'High', label: '高优先级', color: 'text-red-600', bgColor: 'bg-red-50 hover:bg-red-100' },
    { value: 'Medium', label: '中优先级', color: 'text-yellow-600', bgColor: 'bg-yellow-50 hover:bg-yellow-100' },
    { value: 'Low', label: '低优先级', color: 'text-green-600', bgColor: 'bg-green-50 hover:bg-green-100' },
    { value: null, label: '无优先级', color: 'text-gray-600', bgColor: 'bg-gray-50 hover:bg-gray-100' },
  ];
  ...
  return (
    <div className={classNames('space-y-4', className)}>
      {/* 状态快速切换 */}
      <div>
        <h4 className="text-sm font-medium text-text-primary mb-2">状态切换</h4>
        <div className="grid grid-cols-2 gap-2">
          {columns.map((column) => (
            ...
            >
              {column.name}
              {task.status === column.id && (
                <span className="ml-2 text-xs">当前</span>
              )}
            </button>
          ))}
        </div>
      ...
      {/* 优先级快速切换 */}
      <div>
        <h4 className="text-sm font-medium text-text-primary mb-2">优先级设置</h4>
      ...
      {/* 负责人快速设置 */}
      <div>
        <h4 className="text-sm font-medium text-text-primary mb-2">负责人</h4>
      ...
      {/* 其他操作 */}
      <div>
        <h4 className="text-sm font-medium text-text-primary mb-2">其他操作</h4>
        <div className="space-y-2">
          {onDuplicate && (
            <Button
              variant="ghost"
      ...
      <div className="pt-4 border-t border-border">
        <h4 className="text-sm font-medium text-text-primary mb-2">任务信息</h4>
        <div className="space-y-1 text-xs text-text-secondary">
          ...
          {task.estimatedEffort && (
            <div className="flex justify-between">
              <span>估算工时:</span>
              <span>{task.estimatedEffort}小时</span>
            </div>
          )}
          {task.loggedTime && (
            <div className="flex justify-between">
              <span>实际工时:</span>
              <span>{task.loggedTime}小时</span>
            </div>
          )}
          {task.tags && task.tags.length > 0 && (
            <div className="flex justify-between">
              <span>标签数量:</span>
              <span>{task.tags.length}个</span>
            </div>
          )}
        </div>
      </div>
